...
(function () {
  ...
  function SkillEditor(props) {
    ...
    return h("div", { className: "flex flex-col gap-4" },
      h("div", { className: "flex items-center gap-3" },
        ...
        !readOnly && h(Button, {
          ...
          onClick: function () {
            ...
            setSaving(true);
            apiMutate(name, "delete", {}).then(function () { onBack(); }).catch(function (e) {
      ...
      h(Tabs, { value: tab, onValueChange: setTab },
        h(TabsList, null,
          h(TabsTrigger, { value: "content" }, "Content"),
          h(TabsTrigger, { value: "patch" }, "Patch"),
          h(TabsTrigger, { value: "files" }, "Files"),
          h(TabsTrigger, { value: "info" }, "Info"),
        ),

        // Content tab
        h("div", { className: "pt-4" },
          h(TabsTrigger, { value: "content", style: { display: "none" } }),
          !readOnly && h(Button, {
            size: "sm",
            className: "mb-3",
            onClick: function () {
        ...
        // Patch tab
        h("div", { className: "pt-4" },
          h(TabsTrigger, { value: "patch", style: { display: "none" } }),
          readOnly
        ...
        // Files tab
        h("div", { className: "pt-4" },
          h(TabsTrigger, { value: "files", style: { display: "none" } }),
          !skill.files || skill.files.length === 0
            ? h("p", { className: "text-xs text-muted-foreground" }, "No extra files.")
            : h("div", { className: "flex gap-4" },
                h("div", { className: "w-48 shrink-0" },
                  h("ul", { className: "text-xs space-y-1" },
                    skill.files.filter(function (f) { return f.path !== "SKILL.md"; }).map(function (f) {
                      return h("li", {
                        key: f.path,
                        className: cn(
                          "cursor-pointer px-2 py-1 rounded truncate",
                          selectedFileSt[0] === f.path ? "bg-primary/20 text-primary" : "hover:bg-muted"
                        ),
                        onClick: function () { setSelectedFile(f.path); setFileEditMode(false); },
                        title: f.path,
                      }, f.path);
                    }),
                  ),
                ),
                selectedFileSt[0] && h("div", { className: "flex-1" },
                  h("div", { className: "flex items-center gap-2 mb-2" },
                    h("span", { className: "text-xs font-mono text-muted-foreground" }, selectedFileSt[0]),
                  ),
                  h("pre", { className: "text-xs bg-muted/50 rounded p-4 overflow-auto max-h-[40vh] whitespace-pre-wrap font-mono" },
                    "/* File content — click Edit to view */",
                  ),
                ),
              ),
        ),
        ...
        // Info tab
        h("div", { className: "pt-4" },
          h(TabsTrigger, { value: "info", style: { display: "none" } }),
          h("dl", { className: "grid grid-cols-2 gap-4 text-xs" },
            ...
            usage && h("div", null, h("dt", { className: "text-muted-foreground mb-1" }, "Used"), h("dd", null, String(usage.use_count || 0) + " times")),
            usage && usage.last_activity_at && h("div", null, h("dt", { className: "text-muted-foreground mb-1" }, "Last activity"), h("dd", null, timeAgo(new Date(usage.last_activity_at).getTime() / 1000))),
          ),
        ),
      ),
    );
